...
@section('content')
    ...
    @if (session('status'))
        <div class="mb-4 rounded-lg border border-[#B49C6E] bg-secondary/60 px-4 py-3 text-sm text-[#3D342A]">
            {{ session('status') }}
        </div>
    @endif
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <button
            type="submit"
            class="w-full rounded-lg bg-[#A38B54] px-4 py-2.5 text-sm font-semibold text-secondary hover:bg-[#A38B54]/90 focus:outline-none focus:ring-2 focus:ring-[#A38B54] focus:ring-offset-2"
        >
