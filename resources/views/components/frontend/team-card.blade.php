                    ...
                    <div class="pt-1">
                        @if ($longBio)
                            <div x-data="{ expanded: false }">
                                <p
                                    x-show="!expanded"
                                    class="text-sm text-text-primary/70 dark:text-gray-300 leading-relaxed line-clamp-2 break-words"
                                >{{ $bio }}</p>
                                <p
                                    x-show="expanded"
                                    x-cloak
                                    class="text-sm text-text-primary/70 dark:text-gray-300 leading-relaxed break-words"
                                >{{ $bio }}</p>
                                <button
                                    id="{{ $btnId }}"
                        ...
                        @else
                            <p class="text-sm text-text-primary/70 dark:text-gray-300 leading-relaxed break-words">{{ $bio }}</p>
                        @endif
                    </div>
                @endif
            </div>
                ...
                <div class="w-full mt-auto pt-2">
                    @if ($longBio)
                        <div x-data="{ expanded: false }">
                            <p
                                x-show="!expanded"
                                class="text-sm text-text-primary/70 dark:text-gray-300 leading-relaxed line-clamp-3 break-words {{ $isRtl ? 'text-right' : 'text-left' }} sm:text-center"
                            >{{ $bio }}</p>
                            <p
                                x-show="expanded"
                                x-cloak
                                class="text-sm text-text-primary/70 dark:text-gray-300 leading-relaxed break-words {{ $isRtl ? 'text-right' : 'text-left' }} sm:text-center"
                            >{{ $bio }}</p>
                            <button
                                id="{{ $btnId }}"
                                @click="expanded = !expanded"
                    ...
                    @else
                        <p class="text-sm text-text-primary/70 dark:text-gray-300 leading-relaxed break-words {{ $isRtl ? 'text-right' : 'text-left' }} sm:text-center">{{ $bio }}</p>
                    @endif
                </div>
            @endif
        </div>
