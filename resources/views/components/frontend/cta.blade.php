...
<section {{ $attributes->merge(['class' => 'py-12 md:py-16 bg-primary text-white rounded-3xl p-8 sm:p-12 shadow-xl my-8 relative overflow-hidden']) }}
         x-data="{ inView: false }"
         x-intersect.once="inView = true">
    <!-- Decorative elements -->
    <div class="absolute -top-24 -end-24 w-64 h-64 rounded-full bg-primary-light/20 blur-2xl pointer-events-none" aria-hidden="true"></div>
    <div class="absolute -bottom-24 -start-24 w-64 h-64 rounded-full bg-secondary-light/20 blur-2xl pointer-events-none" aria-hidden="true"></div>
    ...
    <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-start transition-all duration-1000 transform"
        ...
        <div class="space-y-3 max-w-2xl">
            ...
            @if ($title)
                <h2 class="text-2xl sm:text-3xl md:text-4xl font-bold text-white tracking-tight leading-tight">
                    {{ $title }}
                </h2>
            @endif
