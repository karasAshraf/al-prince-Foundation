...
<div x-data="{
                    ...
                    <div x-data="{ removed: false }" x-show="!removed" class="relative group">
                        ...
                        @if($imgId)
                            <button
                                type="button"
                                @click="removed = true"
                                class="absolute -end-1.5 -top-1.5 rounded-full bg-red-500 p-0.5 text-background hover:bg-red-600 shadow-sm transition"
                                title="إزالة الصورة"
                            >
    ...
    {{-- Upload drop zone --}}
    <label class="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-[#B49C6E]/40 bg-secondary px-4 py-5 text-center transition hover:border-[#A38B54] hover:bg-secondary/20">
