...
<x-frontend-layout :title="$title">
    ...
    <div class="max-w-3xl mx-auto space-y-8"
        ...
        <div class="text-center space-y-4">
            ...
            <h1 class="text-3xl sm:text-4xl font-bold text-text-primary dark:text-background leading-tight">
                {{ $title }}
            </h1>
        ...
        <div x-show="success" x-cloak
            ...
            <div class="pt-4">
                <a href="{{ route('surveys.index') }}" class="inline-flex items-center justify-center px-6 py-3 rounded-2xl bg-primary text-background font-semibold hover:bg-primary/95 transition-all active:scale-[0.98]">
        ...
        <form x-show="!success" action="{{ route('surveys.response.store') }}" method="POST"
              class="bg-background dark:bg-gray-800 border border-secondary/20 rounded-3xl p-6 sm:p-10 space-y-8 shadow-sm relative">
            @csrf
            ...
            <div class="rounded-2xl border border-secondary/15 bg-secondary/5 dark:bg-gray-900/40 p-6 space-y-4">
                <h3 class="text-base font-bold text-text-primary dark:text-background flex items-center gap-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="respondent_name" id="respondent_name" required
                               ...
                               class="w-full px-4 py-3 rounded-xl border border-secondary/30 bg-background dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-primary text-sm text-text-primary dark:text-background">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="tel" name="respondent_phone" id="respondent_phone" required
                               ...
                               x-bind:disabled="submitting"
                               class="w-full px-4 py-3 rounded-xl border border-secondary/30 bg-background dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-primary text-sm text-text-primary dark:text-background text-start direction-ltr">
            ...
            <div class="space-y-8">
                @forelse ($questions as $index => $q)
                    ...
                    <div class="space-y-4 p-5 sm:p-6 rounded-3xl bg-background/40 dark:bg-gray-900/40 border border-secondary/15 relative">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <label class="block text-base font-bold text-text-primary dark:text-background leading-snug">
                                {{ $qText }}
                            </label>
                        </div>
                        ...
                        @if (($qType === 'select' || $qType === 'choice') && !empty($qOpts))
                            <div class="grid grid-cols-1 gap-2 pt-1">
                                @foreach ($qOpts as $optIndex => $opt)
                                    ...
                                    @endphp
                                    <label class="flex items-center gap-3 p-3.5 rounded-2xl border border-secondary/20 dark:border-gray-700 bg-background/20 dark:bg-gray-900/20 hover:bg-secondary/30 dark:hover:bg-secondary/10 cursor-pointer transition-all duration-300 text-sm relative group">
                                        <input type="radio" name="answers[{{ $index }}]" value="{{ $optIndex }}"
                                               x-bind:disabled="submitting"
                                               class="peer sr-only" required>
                                        
                                        <!-- Custom radio circle -->
                                        <div class="w-5 h-5 rounded-full border border-secondary/40 bg-background dark:bg-gray-800 flex items-center justify-center shrink-0 peer-checked:border-primary peer-checked:bg-primary transition-all">
                                            <div class="w-2 h-2 rounded-full bg-background scale-0 peer-checked:scale-100 transition-transform"></div>
                                        </div>
                            ...
                            <div class="grid grid-cols-1 gap-2 pt-1">
                                @foreach ($qOpts as $optIndex => $opt)
                                    ...
                                    <label class="flex items-center gap-3 p-3.5 rounded-2xl border border-secondary/20 dark:border-gray-700 bg-background/20 dark:bg-gray-900/20 hover:bg-secondary/30 dark:hover:bg-secondary/10 cursor-pointer transition-all duration-300 text-sm relative group">
                                        ...
                                        <!-- Custom checkbox box -->
                                        <div class="w-5 h-5 rounded-lg border border-secondary/40 bg-background dark:bg-gray-800 flex items-center justify-center shrink-0 peer-checked:border-primary peer-checked:bg-primary transition-all">
                                            <svg class="w-3.5 h-3.5 text-background scale-0 peer-checked:scale-100 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3">
                        ...
                        @elseif ($qType === 'rating')
                            <div class="flex items-center gap-3 pt-1 flex-wrap">
                                @for ($r = 1; $r <= 5; $r++)
                                    <label class="relative flex flex-col items-center justify-center w-12 h-12 rounded-xl border border-secondary/20 dark:border-gray-700 bg-background/20 dark:bg-gray-900/20 hover:bg-secondary/30 dark:hover:bg-secondary/10 cursor-pointer text-text-primary dark:text-gray-200 transition-all duration-300 group">
                        ...
                        @else
                            <textarea name="answers[{{ $index }}]" rows="3" required
                                      ...
                                      class="w-full px-4 py-3 rounded-2xl border border-secondary/30 bg-background dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-primary text-sm text-text-primary dark:text-background"></textarea>
                        @endif
                    </div>
                @empty
            ...
            @if (!empty($questions))
                <div class="pt-4 text-center">
                    <button type="submit"
                            x-bind:disabled="submitting"
                            class="w-full sm:w-auto px-10 py-3.5 rounded-2xl bg-primary text-background font-semibold hover:bg-primary/95 transition-all shadow-md active:scale-[0.98] disabled:bg-gray-400 disabled:cursor-not-allowed inline-flex items-center justify-center gap-2">
