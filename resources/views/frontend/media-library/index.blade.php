...
<div x-data="mediaPreviewModal()" class="w-full">
    ...
    <div x-show="open"
        ...
        <div x-show="open"
             ...
             @click.outside="close()"
             class="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-primary-light/20 dark:border-gray-700">

            {{-- Modal Header --}}
            <div class="sticky top-0 z-10 flex items-center justify-between gap-4 px-6 py-4 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm border-b border-primary-light/10 dark:border-gray-700/50">
                <div class="flex-1 min-w-0">
                    <h2 id="modal-title" class="text-lg font-bold text-text-primary dark:text-gray-100 truncate" x-text="title"></h2>
                ...
                <button @click="close()" type="button" class="shrink-0 w-9 h-9 rounded-xl flex items-center justify-center text-text-primary/60 hover:text-text-primary hover:bg-secondary-light/50 dark:hover:bg-gray-800 transition-colors">
            ...
            <div class="p-6 space-y-8">
                ...
                <template x-if="totalCount === 0">
                    <div class="py-16 flex flex-col items-center gap-3 text-center">
                        <div class="w-16 h-16 rounded-2xl bg-secondary-light/30 flex items-center justify-center text-primary/60">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            <template x-for="(img, idx) in images" :key="idx">
                                <a :href="img.url" target="_blank" rel="noopener noreferrer" class="group relative aspect-video overflow-hidden rounded-xl border border-primary-light/15 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 hover:border-primary/40 hover:shadow-lg transition-all duration-200">
                                    ...
                                    <div class="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
                                        <svg class="w-6 h-6 text-white drop-shadow" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </div>
                                </a>
                            </template>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-4">
                            <template x-for="(vid, idx) in videos" :key="idx">
                                <div class="rounded-xl overflow-hidden border border-primary-light/15 dark:border-gray-700 bg-black">
                                    ...
                                    <div class="px-4 py-2.5 bg-gray-900/80 flex items-center justify-between gap-2">
                                        ...
                                        <a :href="vid.url" target="_blank" rel="noopener noreferrer" class="shrink-0 inline-flex items-center gap-1 text-xs text-primary-light hover:text-secondary-light transition-colors font-medium">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <template x-for="(pdf, idx) in pdfs" :key="idx">
                                <div class="flex items-center gap-3 p-4 rounded-xl border border-red-200/60 dark:border-red-900/40 bg-red-50/50 dark:bg-red-950/20">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-semibold text-text-primary dark:text-gray-100 truncate" x-text="pdf.name"></p>
                                    ...
                                    <div class="flex items-center gap-1.5 shrink-0">
                                        ...
                                        <a :href="pdf.url" :download="pdf.name" class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-primary text-white hover:bg-primary/90 transition-colors">{{ app()->getLocale() === 'ar' ? 'تحميل' : 'Download' }}</a>
                                    </div>
                                </div>
                            </template>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <template x-for="(doc, idx) in documents" :key="idx">
                                <div class="flex items-center gap-3 p-4 rounded-xl border border-blue-200/60 dark:border-blue-900/40 bg-blue-50/50 dark:bg-blue-950/20">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-semibold text-text-primary dark:text-gray-100 truncate" x-text="doc.name"></p>
                                        <span class="text-xs font-bold text-blue-500 uppercase tracking-wider" x-text="doc.ext || 'DOC'"></span>
                                    </div>
                                    <a :href="doc.url" :download="doc.name" class="shrink-0 inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-primary text-white hover:bg-primary/90 transition-colors">{{ app()->getLocale() === 'ar' ? 'تحميل' : 'Download' }}</a>
                                </div>
                            </template>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            <template x-for="(link, idx) in links" :key="idx">
                                <a :href="link.url" target="_blank" rel="noopener noreferrer" class="flex items-center gap-3 p-4 rounded-xl border border-primary-light/20 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-primary/40 hover:shadow-md group transition-all duration-200">
                                    <div class="shrink-0 w-10 h-10 rounded-lg bg-secondary-light/50 dark:bg-gray-700 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/></svg>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-semibold text-text-primary dark:text-gray-100 group-hover:text-primary dark:group-hover:text-primary-light transition-colors truncate" x-text="link.label"></p>
    ...
    <div class="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-10">
            ...
            <form method="GET" action="{{ route('media-library.index') }}" 
                  class="w-full flex flex-col md:flex-row items-stretch md:items-center gap-4 bg-white dark:bg-gray-800 p-4 rounded-xl border border-primary-light/15 dark:border-gray-700 shadow-sm">
        ...
        @if ($items->count() > 0)
            ...
            <div class="mb-14" x-data="{
                ...
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-xl font-bold text-text-primary dark:text-gray-100 flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-2">
                        <button @click="scrollByCard(-1)" type="button" 
                                ...
                                class="w-9 h-9 rounded-full bg-white dark:bg-gray-800 shadow-sm border border-primary-light/20 flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-all" 
        ...
        @else
            {{-- Empty State --}}
            <div class="py-20 text-center flex flex-col items-center justify-center bg-white dark:bg-gray-800 rounded-xl border border-primary-light/15 dark:border-gray-700 shadow-sm">
                ...
                <h3 class="text-lg font-bold text-text-primary dark:text-gray-100 mb-1">
                    {{ app()->getLocale() === 'ar' ? 'لا توجد نتائج مطابقة' : 'No matching results' }}
                </h3>
                <p class="text-sm text-text-muted dark:text-gray-400 max-w-xs">
