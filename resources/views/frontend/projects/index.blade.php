        ...
        @if ($projects->count() > 0)
            ...
            <section class="mb-16" aria-label="{{ $locale === 'ar' ? 'المشروع البارز' : 'Featured Project' }}">
                <div class="text-start mb-6">
                    <h2 class="text-2xl font-black text-[#3D342A] dark:text-white flex items-center gap-2">
                ...
                <article class="relative overflow-hidden bg-white dark:bg-gray-800 rounded-3xl border border-[#A38B54]/10 dark:border-gray-700/60 shadow-sm hover:shadow-xl transition-all duration-300 group">
                    <div class="grid grid-cols-1 lg:grid-cols-12 items-stretch">
                        ...
                        <div class="lg:col-span-7 relative min-h-[300px] lg:min-h-[450px] overflow-hidden">
                            ...
                            <div class="absolute top-4 start-4 z-10">
                                <span class="px-3.5 py-1.5 text-xs font-bold uppercase tracking-wider rounded-full shadow-sm text-white select-none
                                            {{ $fIsOngoing ? 'bg-primary' : 'bg-primary-light' }}">
                        ...
                        <div class="lg:col-span-5 flex flex-col justify-between p-8 sm:p-10 space-y-6">
                            <div class="space-y-4">
                                @if ($fProgram)
                                    <span class="text-xs font-bold uppercase tracking-wider text-primary dark:text-primary-light">
                                        {{ $fProgram }}
                                    </span>
                                @endif
                                
                                <h3 class="text-2xl sm:text-3xl font-black text-[#3D342A] dark:text-white leading-tight hover:text-primary transition-colors">
                            ...
                            {{-- View Project Button --}}
                            <div class="pt-6 border-t border-gray-100 dark:border-gray-700/60">
                                <a href="{{ $fDetailUrl }}" class="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-primary hover:bg-primary-dark text-white font-bold hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 shadow-md hover:shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-primary">
            ...
            @if ($otherProjects->count() > 0)
                <section aria-label="{{ $locale === 'ar' ? 'مشاريعنا الأخرى' : 'Other Projects' }}">
                    <div class="text-start mb-6">
                        <h2 class="text-2xl font-black text-[#3D342A] dark:text-white flex items-center gap-2">
                            <span class="w-1.5 h-6 bg-primary-light rounded-full"></span>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"
                        ...
                        @foreach ($otherProjects as $index => $project)
                            ...
                            <article class="flex flex-col bg-white dark:bg-gray-800 rounded-3xl overflow-hidden border border-[#A38B54]/10 dark:border-gray-700/60 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 group"
                                ...
                                {{-- Card Image Wrapper --}}
                                <div class="relative aspect-[16/10] overflow-hidden bg-gray-100 dark:bg-gray-900 shrink-0">
                                    @if ($img)
                                    ...
                                    <div class="absolute top-4 end-4">
                                        <span class="px-3 py-1 text-[10px] sm:text-xs font-bold uppercase tracking-wider rounded-full shadow-sm text-white select-none
                                                    {{ $isOngoing ? 'bg-primary' : 'bg-primary-light' }}">
                                ...
                                <div class="flex-grow flex flex-col p-6 sm:p-7 space-y-4">
                                    @if ($program)
                                        <span class="text-xs font-bold uppercase tracking-wider text-primary dark:text-primary-light select-none">
                                            {{ $program }}
                                        </span>
                                    @endif

                                    <h3 class="text-lg sm:text-xl font-bold text-[#3D342A] dark:text-white leading-snug group-hover:text-primary transition-colors duration-200">
                                    ...
                                    {{-- Separator Line --}}
                                    <div class="h-px w-full bg-gray-100 dark:bg-gray-700/60 pt-2" aria-hidden="true"></div>

                                    {{-- CTA Button --}}
                                    <div class="flex items-center justify-between pt-2">
                                        <a href="{{ $detailUrl }}" class="inline-flex items-center gap-1.5 text-xs sm:text-sm font-bold text-[#3D342A] dark:text-white hover:text-primary dark:hover:text-primary-light transition-colors group/btn focus:outline-none">
