<x-frontend-layout title="{{ __('frontend.organizational_structure') }}">
    ...
    <nav class="flex items-center gap-2 text-sm text-text-primary/60 dark:text-gray-400 mt-8 mb-6 justify-center" aria-label="Breadcrumb">
        <a href="{{ route('home') }}" class="hover:text-[#A5780A] transition-colors duration-200">{{ __('frontend.home') }}</a>
        <span class="text-text-primary/30">/</span>
        <a href="{{ route('about.index') }}" class="hover:text-[#A5780A] transition-colors duration-200">{{ __('frontend.about_foundation') }}</a>
        <span class="text-text-primary/30">/</span>
        <span class="text-[#A5780A] font-bold">{{ __('frontend.organizational_structure') }}</span>
    </nav>
    ...
    <div class="relative text-center pb-6 mb-8 sm:mb-12">
        <div aria-hidden="true" class="absolute inset-0 -z-10 flex items-center justify-center pointer-events-none">
            <div class="w-72 h-72 sm:w-96 sm:h-96 rounded-full bg-gradient-to-br from-[#B8974F]/20 via-[#A5780A]/10 to-transparent blur-3xl opacity-60"></div>
        </div>

        <span class="text-xs sm:text-sm font-bold uppercase tracking-wider text-[#A5780A] mb-3 block">
            {{ __('frontend.about_foundation') }}
        </span>

        <h1 class="text-3xl sm:text-4xl lg:text-5xl font-black text-[#372828] dark:text-white leading-tight tracking-tight mb-4">
            {{ $title ?: __('frontend.organizational_structure') }}
        </h1>

        <div class="flex items-center justify-center gap-1.5 mt-4 mb-6" aria-hidden="true">
            <div class="w-3 h-1.5 rounded-full bg-[#A5780A]"></div>
            <div class="w-1.5 h-1.5 rounded-full bg-[#B8974F]"></div>
            <div class="w-6 h-1 rounded-full bg-[#AC8321]"></div>
        </div>

        @if ($desc)
            <p class="mt-4 text-base sm:text-lg text-[#695956] dark:text-gray-300 max-w-2xl mx-auto leading-relaxed">
                {{ $desc }}
            </p>
        @endif
    </div>
    ...
    <x-frontend.section index="0" align="center" class="pt-0">
        @if ($imgUrl)
            <div x-data="{ open: false }" class="relative w-full max-w-5xl mx-auto" @keydown.escape.window="open = false">
                ...
                <div class="relative group rounded-3xl border-2 border-[#A5780A]/20 bg-white dark:bg-gray-800 shadow-xl hover:shadow-2xl hover:border-[#A5780A]/40 transition-all duration-300 overflow-hidden p-3 sm:p-6">
                    
                    {{-- Hint overlay --}}
                    <div class="absolute inset-0 bg-[#372828]/5 group-hover:bg-[#372828]/0 transition-colors duration-300 pointer-events-none z-10"></div>
                    ...
                    <div class="mt-4 flex flex-wrap items-center justify-between gap-4 text-xs font-semibold text-[#695956] dark:text-gray-400 border-t border-[#A5780A]/10 pt-4">
                        <span class="flex items-center gap-1.5">
                            <svg class="w-4 h-4 text-[#A5780A]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"/>
                            </svg>
                            <span>{{ __('frontend.pinch_to_zoom') ?? (app()->getLocale() === 'ar' ? 'اسحب أو قرّب للاستكشاف على الجوال' : 'Pinch/scroll to explore on mobile') }}</span>
                        </span>

                        <button @click="open = true" type="button" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#A5780A] text-[#F5F5F5] hover:bg-[#AC8321] active:scale-[0.98] transition-all">
                            ...
                            <span>{{ __('frontend.view_fullscreen') ?? (app()->getLocale() === 'ar' ? 'عرض ملء الشاشة' : 'View Fullscreen') }}</span>
                        </button>
                    </div>
                </div>
                ...
                <template x-teleport="body">
                    <div 
                        ...
                        class="fixed inset-0 z-[100] flex items-center justify-center bg-[#372828]/95 backdrop-blur-md p-4 sm:p-8"
                        style="display: none;"
                        role="dialog"
                        aria-modal="true"
                    >
                        ...
                        <button 
                            @click="open = false" 
                            type="button" 
                            class="absolute top-4 end-4 z-[110] p-3 rounded-full bg-white/10 text-[#F5F5F5] hover:bg-white/20 transition-all focus:outline-none focus:ring-2 focus:ring-[#A5780A]"
