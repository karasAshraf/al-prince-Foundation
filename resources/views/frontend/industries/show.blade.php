@php
    ...
    $img    = \App\Helpers\MediaHelper::url($industry, 'industry_images', 'image', 'detail');

    $seo    = $industry->seoMeta;
    $metaDesc = $seo ? ($locale === 'ar' ? $seo->meta_description_ar : ($seo->meta_description_en ?? $seo->meta_description_ar)) : null;
@endphp

<x-frontend-layout :model="$industry">

    <div class="max-w-4xl mx-auto space-y-8">
        <!-- Back Navigation -->
        <div>
            <x-frontend.button :href="route('industries.index')" variant="ghost" size="sm">
                {{ app()->getLocale() === 'ar' ? '→' : '←' }} {{ __('frontend.back_to_industries') }}
            </x-frontend.button>
        </div>
