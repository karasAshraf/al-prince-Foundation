@php
    ...
    $title    = $locale === 'ar' ? ($event->title_ar ?: '') : ($event->title_en ?: $event->title_ar);
    $content  = $locale === 'ar' ? ($event->description_ar ?: '') : ($event->description_en ?: $event->description_ar);
    $img      = \App\Helpers\MediaHelper::url($event, 'featured_image', 'image', 'detail');
    $seo      = $event->seoMeta;
    $metaDesc = $seo ? ($locale === 'ar' ? ($seo->meta_description_ar ?: '') : ($seo->meta_description_en ?: $seo->meta_description_ar)) : '';
@endphp

<x-frontend-layout :model="$event">

    <div class="max-w-4xl mx-auto space-y-8">
        <!-- Back Navigation -->
        <div>
            <x-frontend.button :href="route('events.index')" variant="ghost" size="sm">
                {{ app()->getLocale() === 'ar' ? '→' : '←' }} {{ __('frontend.back_to_events') }}
            </x-frontend.button>
        </div>
        ...
        <article class="bg-background dark:bg-gray-800 border border-secondary/20 rounded-3xl p-6 sm:p-10 space-y-6 shadow-sm">
            ...
            @if($galleryItems->count() > 0)
                <div class="pt-6 border-t border-secondary/20">
                    <h3 class="text-lg font-bold text-text-primary dark:text-background mb-4">{{ __('frontend.event_gallery') }}</h3>
